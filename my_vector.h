...
struct my_vector{
    ...
    //Constructor of my_vector is noexcept if Allocator() is noexcept
     my_vector() noexcept(noexcept(Allocator())){
        m_data = nullptr;
        m_size = 0;
        m_cap = 0;
    }
     ...
     explicit my_vector(size_type count, const Allocator& alloc = Allocator()) : m_alloc(alloc){
        m_size = count;
        m_cap = count;
     ...
     my_vector(size_type count, const T& value, const Allocator& alloc = Allocator()) : m_alloc(alloc){
        m_size = count;
        m_cap = count;
     ...
     my_vector(const my_vector& other, const Allocator& alloc) : m_alloc(alloc){
        m_size = other.m_size;
        m_cap = other.m_cap;
        m_data = nullptr;
        if(other.m_data){
     ...
     my_vector(my_vector&& other) noexcept : m_alloc(std::move(other.m_alloc)){
        m_size = other.m_size;
        m_cap = other,m_cap;
        m_data = other.m_data;
     ...
     my_vector(my_vector&& other, const Allocator& alloc) {
        m_alloc = alloc;
        m_size = other.m_size;
        m_cap = other.m_cap;
    ...
    my_vector( InputIt first, InputIt last, const Allocator& alloc = Allocator()) : m_alloc(alloc){
     ...
     my_vector(std::initializer_list<T> init, const Allocator& alloc = Allocator())
    :my_vector(init.begin(), init.end(), alloc){}

    template<class U, class Al>
    const bool _check_propagation(const my_vector<T, Al> vec){
...
//        }
    ...
    template< std::input_iterator InputIt >
     iterator insert(const_iterator pos, InputIt first, InputIt last){
        ...
        size_t j = pos - m_data;
        if(count == 0) [[unlikely]] return m_data + j;
        reserve(m_size+count);
        for(size_t i = m_size-1;i>=j;i--){
            std::construct_at(&m_data[i+count], std::move(m_data[i]));
            std::destroy_at(&m_data[i]);
    ...
    reference emplace_back( Args&&... args ){
     ...
     reference at(size_type pos){
        if(pos >= size())[[unlikely]] throw std::out_of_range("my_vector::at");
        return *(m_data + pos);
    }
